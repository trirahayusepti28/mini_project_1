...
class LinkedList:
    ...
    # Fungsi untuk mencari barang berdasarkan ID menggunakan Jump Search
    def jump_search_by_id(self, id_barang):
        if not self.head:
            print("-- Inventaris barang kosong --")
            return None
    ...
    # Fungsi untuk mencari barang berdasarkan nama barang menggunakan Jump Search
    def jump_search_by_name(self, nama_barang):
        if not self.head:
            print("-- Inventaris barang kosong --")
            return None
        ...
        prev = None
        current = self.head
        while current and current.nama_barang < nama_barang:
            prev = current
            for _ in range(min(jump, n)):
                current = current.next
                if not current:
        ...
        if current and current.nama_barang == nama_barang:
            print(f"\nBarang dengan nama {nama_barang} ditemukan:")
            print(f"ID Barang: {current.id_barang}, Nama Barang: {current.nama_barang}, Jumlah: {current.jumlah}, Kondisi: {current.kondisi}, Lokasi: {current.lokasi}, Nama Pemilik: {current.nama_pemilik}")
            return current
        else:
    ...
    def quick_sort(self, arr, low, high, key, reverse=False):
        if low < high:
            pi = self.partition(arr, low, high, key, reverse)
            self.quick_sort(arr, low, pi - 1, key, reverse)
            self.quick_sort(arr, pi + 1, high, key, reverse)

    def partition(self, arr, low, high, key, reverse):
        pivot = arr[high]
        i = low - 1

        for j in range(low, high):
            if not reverse:
                if getattr(arr[j], key) <= getattr(pivot, key):
                    i += 1
    ...
    def sort_barang(self, key, reverse=False):
        if not self.head:
            print("-- Inventaris barang kosong --")
            return
        ...
        self.quick_sort(arr, 0, len(arr) - 1, key, reverse)

        table = PrettyTable()
        table.field_names = ["ID", "Nama Barang", "Jumlah", "Kondisi", "Lokasi", "Nama Pemilik"]
        for node in arr:
            table.add_row([node.id_barang, node.nama_barang, node.jumlah, node.kondisi, node.lokasi, node.nama_pemilik])
        print(table)
...
def cari_barang():
    while True:
        ...
        opsi_pencarian = input("Pilih opsi (1/2/3): ")
        if opsi_pencarian == "1":
            id_yang_dicari = input("Masukkan ID yang ingin dicari: ")
            inventaris.jump_search_by_id(id_yang_dicari)
            break  
        elif opsi_pencarian == "2":
            nama_yang_dicari = input("Masukkan nama yang ingin dicari: ")
            inventaris.jump_search_by_name(nama_yang_dicari)
            break  
        elif opsi_pencarian == "3":
            break  
        else:
...
# Fungsi untuk melakukan sorting barang oleh admin
def sort_barang():
    print("\n=== Sort Barang ===")
    print("1. Ascending")
    print("2. Descending")

    pilihan_sort = input("Pilih jenis sorting (1/2): ")
    if pilihan_sort == "1":
        print("\n=== Sort Barang Ascending ===")
        print("1. ID Barang")
        print("2. Nama Barang")
        print("3. Nama Pemilik")
        sort_key = input("Pilih kunci sorting (1/2/3): ")
        if sort_key == "1":
            inventaris.sort_barang('id_barang')
        elif sort_key == "2":
            inventaris.sort_barang('nama_barang')
        elif sort_key == "3":
            inventaris.sort_barang('nama_pemilik')
        else:
            print("Pilihan tidak valid.")
    elif pilihan_sort == "2":
        print("\n=== Sort Barang Descending ===")
        print("1. ID Barang")
        print("2. Nama Barang")
        print("3. Nama Pemilik")
        sort_key = input("Pilih kunci sorting (1/2/3): ")
        if sort_key == "1":
            inventaris.sort_barang('id_barang', reverse=True)
        elif sort_key == "2":
            inventaris.sort_barang('nama_barang', reverse=True)
        elif sort_key == "3":
            inventaris.sort_barang('nama_pemilik', reverse=True)
        else:
            print("Pilihan tidak valid.")
    else:
        print("Pilihan tidak valid.")
...
def pinjam_barang(customer):
    ...
    barang = inventaris.jump_search_by_id(id_barang)
    if barang:
        if barang.jumlah > 0:
...
def customer_menu(customer):
    while True:
        ...
        print("3. Cari Barang")
        print("4. Lihat Riwayat Pinjaman")
        print("5. Keluar")
...
# Fungsi untuk melakukan pencarian barang oleh customer
def search_barang_customer():
    while True:
        print("\n=== Pencarian Barang ===")
        print("1. Cari berdasarkan ID")
        print("2. Cari berdasarkan Nama")
        ...
        opsi_pencarian = input("Pilih opsi (1/2/3): ")
        if opsi_pencarian == "1":
            id_barang_dicari = input("Masukkan ID barang yang ingin dicari: ")
            inventaris.jump_search_by_id(id_barang_dicari)
            break  # Keluar dari loop setelah pencarian selesai
        elif opsi_pencarian == "2":
            nama_barang_dicari = input("Masukkan nama barang yang ingin dicari: ")
            inventaris.jump_search_by_name(nama_barang_dicari)
            break  # Keluar dari loop setelah pencarian selesai
        elif opsi_pencarian == "3":
